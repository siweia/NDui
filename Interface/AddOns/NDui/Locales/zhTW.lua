...
L["Action*"] = "行為*"
L["Action Intro"] = "|n為團隊框架設置按鍵行為。|n|n輸入target，設置目標；|n|n輸入focus，設置焦點；|n|n輸入follow，設置跟隨；|n|n輸入數字，設置施法的法術ID；|n|n你也可以直接輸入巨集。|n對於存在多行内容的巨集，用\\n來進行換行。|n|n你可以使用巨集來為不同專精分別設置:|n/cast [@mouseover,spec1]XXX;[@mouseover,spec2]XXX"
L["Key*"] = "按鍵*"
