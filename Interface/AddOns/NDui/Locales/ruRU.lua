...
L["AWConfigTips"] = "|nБольше подсказок при наведении на заголовок настроек.|n|nНеобходима перезагрузка интерфейса после изменения настроек.|n|nYou can hold CTRL+ALT, and click AuraWatch icon to add spell into ignore list, or click UnitAura icon to remove spell from ignore list." -- need translation
L["Groups"] = "Группы"
