...
L["Tutorial Page1"] = "Приветствую вас в обучении! Оно необходимо, чтобы облегчить вам настройку NDui.|n|nНажмите кнопку, чтобы установить некоторые настройки для NDui по умолчанию, включая индикаторы здоровья,панели команд, рамки рейда.|n|n|cffff0000These options cannot be skipped.|r" -- need translation
...
L["AutoQuestTip"] = "|nIf checked, quests will be interacted automatically.|nGossip window with one single option will be auto selected as well.|n|nYou can hold key SHIFT to temporary stop automation." -- need translation
...
L["AWConfigTips"] = "|nБольше подсказок при наведении на заголовок настроек.|n|nНеобходима перезагрузка интерфейса после изменения настроек.|n|nYou can hold CTRL+ALT, and click AuraWatch AuraID icon to add spell into ignore list, or click UnitAura icon to remove spell from ignore list." -- need translation
L["Groups"] = "Группы"
...
L["Option* Tips"] = "|nНастройки не отмеченные звёздочкой (*) требуют перезагрузки интерфейса.|n|nDouble click on slider header and color picker block would reset their value.|n|nКлик на иконку шестерёнки откроет дополнительные настройки.|n|nДля большей информации используйте команду /ndui." -- need translation
...
L["ChatFilterWhiteListTip"] = "|nПоказывать сообщения которые содержат нижеприведённый слова. Отключено если пусто."
L["FilterListTip"] = "|nОтфильтровывает сообщения, содержащие эти слова."
L["CustomUnitTips"] = "|nEnter unit name or npc ID.|nYou can see unit npc ID in GameTooltip by holding KEY SHIFT." -- need translation
...
L["JunkMode Enabled"] = "|nКлик, чтобы отметить вещи как хлам.|nЕсли 'Автопродажа хлама' включена, эти вещи также будут проданы.|nСписок сохраняется на весь аккаунт, and won't be in the export data.|nYou can hold CTRL+ALT and click to wipe the custom junk list." -- need translation
...
L["ChatBGType"] = "Тип фонового чата"
...
L["ExecuteRatio"] = "Коэффициент исполнения" -- хз
L["ExecuteRatioTip"] = "|nЕсли процент работоспособности юнита ниже установленного вами лимита выполнения, цвет текста его имени станет красным.|nИндикатор выполнения будет отключен при 0."
...
L["MapRevealGlow"] = "Карта показывает тень"
L["MapRevealGlowTip"] = "|nЕсли этот флажок установлен, как только вы включите отображение карты, неизведанные области останутся под туманом войны."
...
L["DeleteUnitProfileTip"] = "|nВведите имя персонажа, профиль которого вы собираетесь удалить, формат ввода - 'Имяперсонажа-Имясервера'. Вам нужно ввести имя только в том случае, если персонаж находится в том же мире, что и вы. |N|nЭто также удалит информацию о золоте. |N|nНажмите клавишу ESC, чтобы очистить поле редактирования, нажмите клавишу Enter, чтобы подтвердить."
L["Delete unit profile?"] = "Вы уверены что хотите удалить %s%s|r ниформацию профиля?"
...
L["SendActionCDTip"] = "|nЕсли включено, вы можете прокрутить колесико мыши на кнопке панели действий и отправить ее статус восстановления в свою группу. |n|nДоступно только для панели действий NDui по умолчанию."
L["Contact"] = "Контакты"
...
L["ChatScrollHelp"] = "Прокрутите несколько строк, удерживая клавишу Ctrl, и прокрутите вверх или вниз, удерживая клавишу Shift."
