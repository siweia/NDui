...
function M:ReplaceFindGroupButton()
	...
	local filterIDs = {}
	local menuList = {
		[1] = {text = "过滤地下城", isTitle = true, notCheckable = true},
		[2] = {text = "开关", notCheckable = true, keepShownOnClick = true, func = function()
			allOn = not allOn
			for i = 0, 9 do
				mapData[i].isOff = allOn
	...
	local function onCheck(self)
		return not mapData[self.arg1].isOff
	end

	local function onClick(self, index, aID)
		mapData[index].isOff = not mapData[index].isOff
		filterIDs[aID] = mapData[index].isOff
		LFGListSearchPanel_DoSearch(searchPanel)
	end
	...
	searchPanel.RefreshButton:HookScript("OnMouseDown", function(self, btn)
		...
		EasyMenu(menuList, B.EasyMenu, self, 20, 0, "MENU")
	end)
